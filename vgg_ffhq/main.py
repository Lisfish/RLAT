...
# 拼接数据集路径
dataset_path = current_dir / "small_sample" / "test"
CONFIG = {
    "MODEL_PATH": "best_vgg16.pth",
    "DATA_DIR": dataset_path,  # 指向包含 fake/real 的上一级目录
    "IMG_SIZE": 150,
    "MASK_SIZE": 15,
    "EPSILON": 1.0,
    "EPS_DECAY": 0.995,
...
def denormalize_and_save(img_tensor, save_path):
    """反归一化图像张量并保存"""
    img_tensor = img_tensor * 0.5 + 0.5  # 匹配 0.5 的归一化
...
def main():
    ...
    # 获取类别映射名称 (例如: {0: 'fake', 1: 'real'})
    class_names = env.dataset.classes
    print(f"类别映射已识别: {env.dataset.class_to_idx}")

    # 定义 DQN 维度
    input_size = 8192  # 对应 VGG16 特征层输出展平后的维度
    output_size = (CONFIG["IMG_SIZE"] // CONFIG["MASK_SIZE"]) ** 2  # 100个网格
    ...
    print("\n" + "=" * 50)
    print(f"训练评估完成！")
    print(f"原始模型准确率 (Original Accuracy): {orig_acc:.2f}%")
